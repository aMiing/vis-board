...
<template id="my-commanTitle">
	<div ref="drag" class="drag commonTitle" @mousedown="MoveIt" title="通用标题" :uid="item.uid" 
		...
		<div ref="titleText" style="position:absolute;width:100%;display:table;text-align: center;height: 100%;" 
			...
			<div style="display:table-cell;vertical-align: middle;">
				<a class="connect" :href="item.url" :target="item.target"  :style="{color:item.fontColor}">{{item.text}}</a>
			</div>
...
<script>
	export default {
		...
		created:function(){
			if(this.item.url == ''){
				this.item.url = 'javascript:void(0)';
				console.log('mmm');
			}
		...
		mounted:function(){
			...
			/*阻止在该配置页面出发a链接跳转事件，此事件根据需求保留或删除*/
			// console.log(document.getElementsByClassName('connect')[0]);
			document.getElementsByClassName('connect')[0].onclick = function(){
				return false;
			}
		},
		methods: {
