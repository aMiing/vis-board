...
<template id="my-scrollText">
	<div ref="drag" class="drag scrollText" @mousedown="MoveIt" title="滚动文本" :uid="item.uid" 
		...
		<div ref="titleText" style="position:absolute;width:100%;display:table;height: 100%;">
			...
			:style="{fontWeight:item.fontWeight*10,fontFamily:item.fontValue,fontSize:item.fontSize/20+'vw',color:item.fontColor,background: item.bgcolor}">
		        	<marquee :behavior="item.behavior" :direction="item.direction" :scrollamount="item.scrollamount">
		        		{{item.text}}
		        	</marquee>
			</div>
		</div>
