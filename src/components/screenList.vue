<template>
  <div class="list-content">
    ...
    <el-tabs
      tab-position="left"
      style="height: 100%; background-color: #171b22; padding-top: 5px"
    >
      <el-tab-pane>
        <span slot="label" style="display: block; width: 9vw; text-align: left"
          ><i class="el-icon-menu"></i>全部大屏</span
        >
            ...
            <el-card :body-style="{ padding: '0px' }">
              ...
              <div>
                ...
                >
                  ...
                  <i
                    ...
                    @click="deleatePannel(pannel.id)"
                  ></i>
                  <el-button
                    type="primary"
                    class="edit"
                    @click="openWindow(pannel.id)"
                    >编辑</el-button
                  >
                </div>
              ...
              <div style="padding: 5px 10px 10px 10px">
                <div class="titlecontain" style="font-size: 16px">
                  <i
                    class="el-icon-edit"
                    @click="modifyPannelTitle(index)"
                    title="编辑名称"
                  ></i>
                  <span style="line-height: 36px; padding-left: 4px">{{
                    pannel.title
                  }}</span>
                </div>
      ...
      <el-tab-pane>
        <span slot="label" style="display: block; width: 9vw; text-align: left"
          ><i class="el-icon-menu"></i>未分组</span
        >
      </el-tab-pane>
    </el-tabs>
        ...
        this.$confirm("发布之后可以通过链接分享和访问, 是否继续?", "提示", {
          ...
          .then(() => {
            ...
            this.$http
              ...
              .catch((err) => {
                console.log(
                  "未能成功发送publishedOr(1)数据，请联系系统维护人员或稍后重试！"
                );
                return;
              });
          })
        ...
        this.$confirm("取消发布之后不能通过链接分享和访问, 是否继续?", "提示", {
          ...
          .then(() => {
            ...
            this.$http
              ...
              .catch((err) => {
                console.log(
                  "未能成功发送publishedOr(0)数据，请联系系统维护人员或稍后重试！"
                );
                return;
              });
          })
      ...
      this.$prompt("请输入新标题", "修改屏幕标题", {
        ...
        .then(({ value }) => {
          ...
          this.$http
            .post(
              url,
              { id: this.panels[index].id, value: value },
              { emulateJSON: true }
            )
            .then((req, res) => {
            ...
            .catch((err) => {
              alert(
                "页面数据可能与数据库不完全匹配，请刷新页面或联系系统维护人员！"
              );
              return;
            });
          //定义一个提交修改的函数，这里调用
